...
class ToolCell(Cell):
    ...
    def renderable(self):
        ...
        head = Text.from_markup(
            f"{glyph} [blue]{self.name}[/]  [dim]({self.status})[/]"
        )
        if self.output:
            return Group(
                head, Text("    " + self.output.replace("\n", "\n    "), style="dim")
            )
        return head
...
class SpinnerCell(Cell):
    ...
    def renderable(self):
        return Text.from_markup(
            f"[yellow]{self.FRAMES[self.frame]}[/] [dim]{self.label}…[/]"
        )
...
class Session:
    def __init__(
        ...
        self.commit_cb: Callable[[Cell], None] = (
            lambda c: None
        )  # print cell to scrollback
    ...
    def _on_cell_update(self, cell: Cell) -> None:
        ...
        if (
            isinstance(cell, ToolCell)
            and cell.status in ToolCell.TERMINAL
            and cell in self.live
        ):
            self._commit(cell)
        else:
            self.invalidate()
...
async def run_harness() -> int:
    ...
    async def handler(ui: Session, prompt: str):
        ...
        if prompt == "LONG":
            ...
            state.update(
                card=card,
                inv_delta=ui.invalidate_count - inv_before,
    ...
    check(
        "1. type-ahead: 2nd prompt queued, not dropped",
        queued,
        f"depth={session.queue_depth}",
    )
    ...
    interrupt_markers = sum(
        1
        for c in session.committed
        if isinstance(c, TextCell) and c.role == "system" and "interrupted" in c.text
    )
    check(
        "2. exactly ONE interrupt marker (no duplicate)",
        interrupt_markers == 1,
        f"markers={interrupt_markers}",
    )
    check(
        "2. session survived interrupt (queued turn ran)",
        "QUEUED" in state["ran"],
        f"ran={state['ran']}",
    )
    ...
    check(
        "3. tool card was LIVE (mutable) while running", state.get("live_while_running")
    )
    check(
        "3. mutable cell: handle flipped running->done", card and card.status == "done"
    )
    check(
        "3. mutated cell re-renders as 'done'",
        "done" in after and "running" not in after,
    )
    check(
        "3. finalized cell COMMITTED to scrollback (selectable)",
        state.get("committed_after"),
    )
    check(
        "3. .update() triggered a redraw",
        state.get("inv_delta", 0) >= 1,
    ...
    check(
        "4. inline image rendered as cell (half-block)",
        len(img_lines) > 3 and has_color,
    ...
    print(
        ...
        + (
            ...
            if ok
            else "\x1b[31mSOME GATES FAILED\x1b[0m"
        )
    )
...
def run_probe() -> int:
    ...
    print(
        "  Phase 0 uses half-block fallback (selectable text-cells); true kitty/iTerm/sixel = Phase 4."
    )
    return 0
...
def run_interactive() -> int:
    ...
    class LiveTailControl(UIControl):
        ...
        def create_content(self, width, height):
            ...
            return UIContent(
                get_line=get_line, line_count=len(lines), show_cursor=False
            )
    ...
    status_win = Window(
        content=FormattedTextControl(status_text), height=1
    )  # no solid bg
    composer_win = Window(
        content=BufferControl(buffer=composer),
        height=1,
        get_line_prefix=lambda *a: [("class:prompt", " > ")],
    )
    sep_win = Window(
        height=1, char="─", style="class:sep"
    )  # border line for separation
    # order top->bottom: live tail, separator, composer, status bar below the input
    layout = Layout(
        HSplit([live_win, sep_win, composer_win, status_win]),
    ...
    async def main():
        ...
        s.message(
            "system",
            "inline spike — finalized lines are selectable scrollback; the "
